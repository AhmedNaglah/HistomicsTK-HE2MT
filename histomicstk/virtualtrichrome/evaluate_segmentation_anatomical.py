...
EXPERIMENT_ID = 'Evaluate_Segmentation_Anatomical_splitted_1206'

f1 = open(f'{PATH}/{EXPERIMENT_ID}.out', 'w')
...
datasets = os.listdir(f"{PATH}/anatomical_features_each")

for dataset in datasets:
    models = os.listdir(f"{PATH}/anatomical_features_each/{dataset}/output")
    for model in models:
        # Load Images at directory
        datadir = f"{PATH}/anatomical_features_each/{dataset}/output/{model}"
        ...
        for i in range(len(fnames)):
            ...
            print(f'{dataset};{model};{fname};{acc};{iou};{iouA};{iouB};{dice};{dice2}', file=f1)
