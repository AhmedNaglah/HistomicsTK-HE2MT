...
EXPERIMENT_ID = 'cycleGANEnsemble_2'
...
for elem in ensemble:
    ...
    if folder.endswith('1024'):
        reference = (folder, size)
    else:
        others.append((folder, size))

folder_ref, size_ref = reference
...
for fname in fnames:
    ...
    images = [cv2.imread(f"{PATH}/{folder_ref}/{fname}")]
    ...
    cv2.imwrite(f"{PATH}/{EXPERIMENT_ID}/{fname}", blended_image)