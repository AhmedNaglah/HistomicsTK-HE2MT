...
EXPERIMENT_ID = 'Evaluate_Segmentation_Anatomical_splitted_extended_256_Monday'

f1_bile = open(f'{PATH}/bile.csv', 'w')
f1_portal = open(f'{PATH}/portal.csv', 'w')
f1_hepatic = open(f'{PATH}/hepatic.csv', 'w')
header = "dataset,model,fname,ACC,iou,iouA,iouB,dice,DSC"
print(header, file=f1_bile)
print(header, file=f1_portal)
print(header, file=f1_hepatic)
...
SUBROOT = "anatomical_features_each_256"

datasets = os.listdir(f"{PATH}/{SUBROOT}")
dataset_map = {
    "hepatic": "Hepatic Artery Branch",
    "portal": "Portal Vein Branch",
    "bile": "Bile Duct Branch",
}

for dataset in datasets:
    models = os.listdir(f"{PATH}/{SUBROOT}/{dataset}/output_256")
    for model in models:
        # Load Images at directory
        datadir = f"{PATH}/{SUBROOT}/{dataset}/output_256/{model}"
        ...
        for i in range(len(fnames)):
            ...
            model2 = model.replace(" ","").replace("_", "-").replace("GAN-", "GAN+").replace("HE", "HE2F")
            dataset2 = dataset_map[dataset]
            if dataset=="bile":
                print(f'{dataset2},{model2},{fname},{acc},{iou},{iouA},{iouB},{dice},{dice2}', file=f1_bile)
            elif dataset=="portal":
                print(f'{dataset2},{model2},{fname},{acc},{iou},{iouA},{iouB},{dice},{dice2}', file=f1_portal)
            elif dataset=="hepatic":
                print(f'{dataset2},{model2},{fname},{acc},{iou},{iouA},{iouB},{dice},{dice2}', file=f1_hepatic)
