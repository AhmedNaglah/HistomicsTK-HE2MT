...
PATH = "F:/N002-Research/liver-pathology/segmentation/validation_samples/"
EXPERIMENT_ID = 'validation_samples' 
...
outdirHE = f'{PATH}/output/HE'
outdircGAN_MT2F_CLR = f'{PATH}/output/cGAN_MT2F_CLR'
outdircGAN_MT2F_UNET = f'{PATH}/output/cGAN_MT2F_UNET'
outdircycleGAN_MT2F_CLR = f'{PATH}/output/cycleGAN_MT2F_CLR'
outdircycleGAN_MT2F_UNET = f'{PATH}/output/cycleGAN_MT2F_UNET'

createDir(f'{PATH}/output')
createDir(outdirHE)
...
def TF2CV(im):
    ...
    img_ = np.array((img_>128 + 0)*255, dtype='uint8')
...
def process_image(indir, outdir, modelname, fname):
    ...
    out = cv2.hconcat((im, gt, prediction))
