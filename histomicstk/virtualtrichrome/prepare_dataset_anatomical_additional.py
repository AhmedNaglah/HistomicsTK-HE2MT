...
PATH = "F:/N002-Research/liver-pathology/segmentation/anatomical_features/legacy_input_additional"
EXPERIMENT_ID = 'PREPARE-SEGMENTATION-DATASET-ANATOMICAL-Additional'

ftrs = ["bile", "hepatic", "portal"]

for ftr in ftrs:
    input_subdir_cGAN = f'{PATH}/{ftr}/cGAN'
    input_subdir_cycleGAN = f'{PATH}/{ftr}/cycleGAN'

    output_subdir = f'{PATH}/input_'
    createDir(output_subdir)

    createDir(f"{output_subdir}/{ftr}")

    out_he = f"{output_subdir}/{ftr}/HE"
    out_mtcGAN = f"{output_subdir}/{ftr}/MT_cGAN"
    out_mtcycleGAN = f"{output_subdir}/{ftr}/MT_cycleGAN"
    ...
    for i in range(len(fnames)):
        fname = fnames[i]
        processImage(input_subdir_cGAN, (out_he, out_mtcGAN), fname)
        print(f"Processing {i}")

    fnames = os.listdir(input_subdir_cycleGAN)
    for i in range(len(fnames)):
        fname = fnames[i]
        processImage(input_subdir_cycleGAN, (out_he, out_mtcycleGAN), fname)
        print(f"Processing {i}")
